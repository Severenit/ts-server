...
async function startNewGame(settings = {}) {
    try {
        // Создаем новую игру
        const response = await fetch(`${config.apiUrl}/game/new`, {
            method: 'POST',
            headers: {
        ...
        // Если первый ход AI, делаем запрос на ход AI
        if (data.gameState.currentTurn === 'ai') {
            const aiMoveResponse = await fetch(`${config.apiUrl}/game/${data.gameId}/ai-move`);
            ...
            return {
                ...data,
                aiMove: aiMoveData.moveResult,
                gameState: aiMoveData.gameState
            };
        }
...
async function getGameState(gameId) {
    try {
        const response = await fetch(`${config.apiUrl}/game/${gameId}`);
...
async function makePlayerMove(gameId, cardIndex, position) {
    try {
        const response = await fetch(`${config.apiUrl}/game/${gameId}/player-move`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
            body: JSON.stringify({ cardIndex, position })
        });
        
        if (!response.ok) {
            throw new Error('Failed to make player move');
        }
...
async function makeAIMove(gameId) {
    try {
        const response = await fetch(`${config.apiUrl}/game/${gameId}/ai-move`);
